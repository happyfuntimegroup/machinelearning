...
def field_citations_avarage(data, test):
    ...
    for index, i_paper in data.iterrows(): #iterate over dataframe 
        ...
        # iterate over the fields and check if it excists in the dictionary 
        for field in fields:
            if field in fields_dict.keys(): # if the field is in the dict
    ...
    for index, i_paper in data.iterrows(): # iterate over the dataframe 
        fields = i_paper['fields_of_study'] # check all the fields for one paper
